...
def load_and_cache_examples(args, tokenizer, pad_token_label_id, mode):
    ...
    # Update here
    # if os.path.exists(cached_features_file) # and not args.overwrite_cache and (mode != "train" or os.path.exists(cached_features_file_src)):

    if os.path.exists(cached_features_file) and not args.overwrite_cache and (mode != "train" or os.path.exists(cached_features_file_src)):
        logger.info("Loading features from cached file %s", cached_features_file)
        features = torch.load(cached_features_file, map_location=args.device, weights_only=False)
        if mode == "train":
            logger.info("Loading source domain features from cached file %s", cached_features_file_src)
            features_src = torch.load(cached_features_file_src, map_location=args.device, weights_only=False)
    else:
