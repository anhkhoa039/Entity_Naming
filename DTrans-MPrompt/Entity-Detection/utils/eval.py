...
# Removed flashtool import; using standard logging
...
# New here
def evaluate_mention_detection_owner(
    true_chunks_list: List[Set[Tuple[int, int]]],
    pred_chunks_list: List[Set[Tuple[int, int]]],
    logger,
    prefix: str = ""
) -> dict:
    """Evaluate mention detection using OWNER method approach.
    
    This function evaluates mention detection by computing true positives, 
    false negatives, and false positives directly on entity spans (similar to OWNER method).
    
    Args:
        true_chunks_list: List of sets of true entity chunks per sentence.
                          Each chunk is a tuple (start_idx, end_idx).
        pred_chunks_list: List of sets of predicted entity chunks per sentence.
                          Each chunk is a tuple (start_idx, end_idx).
    ...
    for true_chunks, pred_chunks in zip(true_chunks_list, pred_chunks_list):
        # Convert to lists for tracking matched status
        true_list = list(true_chunks)
        pred_list = list(pred_chunks)
        
        # Track matched entities
        true_matched = [False] * len(true_list)
        pred_matched = [False] * len(pred_list)
        
        # Find true positives (matching span boundaries)
        for i_true, true_entity in enumerate(true_list):
            true_start, true_end = true_entity
            for i_pred, pred_entity in enumerate(pred_list):
                pred_start, pred_end = pred_entity
                # Match if start and end positions are the same (mention detection)
                if true_start == pred_start and true_end == pred_end:
                    if not true_matched[i_true] and not pred_matched[i_pred]:
                        tp += 1
                    true_matched[i_true] = True
                    pred_matched[i_pred] = True
        
        # Count false negatives (unmatched true entities)
        fn += sum(1 for m in true_matched if not m)
    ...
    logger.info("***** OWNER Mention Detection Eval results %s *****", prefix)
...
def evaluate_with_owner_method(
    args, 
    out_id_list_bio: List[List], 
    preds_id_list_bio: List[List],
    preds_id_list_tb: List[List],
    preds_id_list_se_s: List[List],
    preds_id_list_se_e: List[List],
    logger,
    prefix: str = "",
    use_ensemble: bool = True
) -> dict:
    """Wrapper function to evaluate using OWNER method on the current model outputs.
    
    This function converts the BIO/TB/SE predictions to chunk format and evaluates
    using the OWNER mention detection approach.
    
    Args:
        args: Arguments containing data_dir and dataset info.
        out_id_list_bio: List of ground truth BIO tag sequences.
        preds_id_list_bio: List of predicted BIO tag sequences.
        preds_id_list_tb: List of predicted Tie-Break tag sequences.
        preds_id_list_se_s: List of predicted Start tag sequences.
        preds_id_list_se_e: List of predicted End tag sequences.
        logger: Logger instance.
        prefix: Prefix for logging.
        use_ensemble: Whether to use ensemble predictions (BIO + TB + SE).
    
    Returns:
        dict: OWNER-style evaluation results.
    """
    ...
    for ground_truth_id_bio, predicted_id_bio, predicted_id_tb, predicted_id_s, predicted_id_e in zip(
        out_id_list_bio, preds_id_list_bio, preds_id_list_tb, preds_id_list_se_s, preds_id_list_se_e
    ):
        # Get ground truth chunks: (start, end) tuples
        lab_chunks_bio = get_chunks(ground_truth_id_bio, tag_to_id(args.data_dir, args.dataset))
        true_chunks_list.append(set(lab_chunks_bio))
        ...
        if use_ensemble:
            ...
            pred_chunks = set(lab_pred_chunks_bio + lab_pred_chunks_se + lab_pred_chunks_tb)
        else:
            lab_pred_chunks_bio = get_chunks(predicted_id_bio, tag_to_id(args.data_dir, args.dataset))
            pred_chunks = set(lab_pred_chunks_bio)
        
        pred_chunks_list.append(pred_chunks)
    
    return evaluate_mention_detection_owner(
        true_chunks_list=true_chunks_list,
        pred_chunks_list=pred_chunks_list,
        logger=logger,
        prefix=prefix
    )
...
def evaluate(args, span_model, tokenizer, id_to_label_span, \
    ...
    # Keep eval_loss as a Python float so it works for both single-GPU and DataParallel
    eval_loss = 0.0
    nb_eval_steps = 0
    preds_bio = None
    preds_se_s = None
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            # In multi-GPU (DataParallel), the losses can be a vector (one per device).
            # Convert to a scalar by averaging and accumulate as a Python float.
            batch_loss = outputs[0] + outputs[2] + outputs[5]
    ...
    # OWNER-style mention detection evaluation
    owner_results = evaluate_with_owner_method(
        args=args,
        out_id_list_bio=out_id_list_bio,
        preds_id_list_bio=preds_id_list_bio,
        preds_id_list_tb=preds_id_list_tb,
        preds_id_list_se_s=preds_id_list_se_s,
        preds_id_list_se_e=preds_id_list_se_e,
        logger=logger,
        prefix=prefix,
        use_ensemble=True
    )
    ...
    # Add OWNER evaluation results
    results.update(owner_results)
